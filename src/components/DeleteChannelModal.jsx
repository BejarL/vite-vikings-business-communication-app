...
import { updateDoc, deleteDoc, doc, arrayRemove, getDocs, collection, query, where } from "firebase/firestore";
...
const removeChannel = async (channelObj) => {
  ...
  if (querySnap.docs[0].role === "creator") {
    ...
    const allMembers = await getDocs(collection(db, `Chats/${channelObj.channelId}/members`));
  ...
  return (
    <div>
      <button onContextMenu={() => setOpenModal(true)} className="flex flex-col items-center w-40 h-full overflow-hidden text-amber-700 bg-orange-300 mb-5 hover:bg-gray-300" >{channel.channelName}</button>
