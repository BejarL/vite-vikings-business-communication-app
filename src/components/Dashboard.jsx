...
function Dashboard() {
  ...
  if (channels) {
    channelElems = channels.map((item) => {
      ...
      return (
        <div key={channel.channelId} className="flex items-center w-full h-12 px-3 mt-2 rounded hover:bg-gray-700 hover:text-gray-300" onClick={() => goToChannel(channel)}>{channel.channelName}</div>
        )
    });
    } 
